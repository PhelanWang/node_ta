...
def memory_scan():
    ...
    # #切换工作目录
#         str = os.popen("find / -name 'memory@_text@_file@.mk'").read();
#         path = os.path.split(str)[0:-1];
#         stt = os.popen("cd /%s" % path[0]).read();
#         os.chdir("%s" % path[0]);
...
#     os.chdir("%s" % path[0]);  
    ...
    path = ['/home/spread/eclipse-workspace/OpenStack-1/openstack-backend/clouds/memory_scan/umuery']
    path = [current_path]
